...
class Main:
    def __init__(self):
        ...
        # self.cached_root_yaw = rb_rot_euler.reshape(*rb_rot_sixd_reshaped.shape[:-2], 3)[:,[0],0]

        # NOTE: run this if we have new animations
        # rb_rot_quat = np.zeros((self.cached_root_yaw.shape[0], 24, 4))
        # for t in range(self.cached_root_yaw.shape[0]):
        #     for i in range(24):

        #         rb_rot = Rotation.from_matrix(rb_rot_rotmat[t,i])
        #         correction = Rotation.from_euler("Z", -1 * self.cached_root_yaw[t][0])
        #         rb_rot = correction * rb_rot
        #         rb_rot_quat[t,i] = rb_rot.as_quat()
        # rb_rot = Rotation.from_matrix(rb_rot_rotmat.reshape(-1,3,3))
        # correction = Rotation.from_euler("Z", -1 * self.cached_root_yaw.reshape(-1,1,1).repeat(24,axis=1).reshape(-1,1))
        # rb_rot = correction * rb_rot
        ...
        self.cached_root_yaw = rb_rot_euler.reshape(
        ...
        self.xs = np.concatenate(
            [self.rb_rot_sixd_inv[self.selected_valid_idxs].reshape(-1, 24, 6)[:, 1:]],
            axis=-1,
        )
        ...
        self.ys = np.concatenate(
            [
                ...
                self.rb_rot_sixd_inv.reshape(-1, 24, 6)[:, 0],
                self.rb_root_vel_inv,
            ],
            axis=-1,
        )
        self.ys = self.ys[self.selected_valid_idxs]
    ...
    def update(self):
        ...
        rb_rot_sixd_reshaped = (
            self.rb_rot_sixd_inv[self.selected_valid_idxs[t]].reshape(24, 3, 2) * 1
        )
        ...
        z[..., 0] += self.engine.get_val("dim_0")
        z[..., 1] += self.engine.get_val("dim_1")
        z[..., 2] += self.engine.get_val("dim_2")
        z[..., 3] += self.engine.get_val("dim_3")
        z[..., 4] += self.engine.get_val("dim_4")
        if z.shape[-1] > 5:
        ...
        cvae_decoded = cvae_decoded.reshape((1, 24, -1)).detach().cpu().numpy()
        tmp = cvae_decoded[:, 0] * 1
        cvae_decoded -= tmp
        yaw_rot = Rotation.from_euler("Z", self.cached_root_yaw[[t], 0])
        ...
        # NOTE this was the reason of problem in legs
        # recon_rot_sixd_reshaped = rb_rot_sixd_reshaped * 1
        # recon_rot_sixd_reshaped = self.rb_rot_sixd_inv[self.selected_valid_idxs[t]].reshape(24, 3, 2) * 1
        # recon_rot_sixd_reshaped[1:] = rb_rot_sixd_recon.reshape(23, 3, 2)

        # add the root yaw back to the root
        # recon_rot_sixd_reshaped = self.sixd_add_root(recon_rot_sixd_reshaped,torch.tensor((self.cached_root_yaw[t][0],0,0)).numpy()).reshape(24, 3, 2)
        ...
        for i in range(len(self.gt_actors)):
            ...
            m[:3, :3] = gt_rotmat
            m[:3, 3] = cvae_decoded_with_yaw[0, i]
