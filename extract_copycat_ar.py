...
def main(args):
    ...
    circle_list = [
        ...
        "0-DanceDB_20180501_CliodelaVara_CLIO_Outsai_poses"
    ]

    
    # for k in d.keys():
    #    # if "walk" in k or "Walk" in k or "run" in k or "Run" in k or "jog" in k or "Jog" in k or "dance" in k or "Dance" in k:
    #     if "dance" in k or "Dance" in k:
    ...
    for index, (k, v) in tqdm(enumerate(d.items()), total=len(d)):
        ...
        # if index % 3000 == 0:
        #     num = int(index / 3000)
        #     out_path = f"torchready_v5_dof_ar_{num}.pkl"
        ...
        rb_poses.append(rb_pos.reshape(1,rb_pos.shape[0], -1))
        rb_rots.append(rb_rot.reshape(1,rb_rot.shape[0],-1))
        root_rots.append(root_rot.reshape(1,root_rot.shape[0],-1))
        rb_pos_invs.append(rb_pos_inv.reshape(1,rb_pos_inv.shape[0], -1))
        rb_rot_sixds.append(rb_rot_sixd.reshape(1,rb_rot_sixd.shape[0], -1))
        rb_rot_sixd_invs.append(rb_rot_sixd_inv.reshape(1,rb_rot_sixd_inv.shape[0], -1))
        rb_vels.append(rb_vel.reshape(1,rb_vel.shape[0], -1))
        rb_vel_invs.append(rb_vel_inv.reshape(1,rb_vel_inv.shape[0], -1))
        rb_angs.append(rb_ang.reshape(1,rb_ang.shape[0], -1))
        dof_vels.append(dof_vel.reshape(1,dof_vel.shape[0], -1))
        dof_poses.append(dof_pos.reshape(1,dof_pos.shape[0], -1))

    # rb_poses = np.stack(rb_poses, axis=0)
    # rb_rots = np.stack(rb_rots, axis=0)
    # root_rots =  np.stack(root_rots, axis=0)
    # rb_pos_invs = np.stack(rb_pos_invs, axis=0)
    # rb_rot_sixds = np.stack(rb_rot_sixds, axis=0)
    # rb_rot_sixd_invs = np.stack(rb_rot_sixd_invs, axis=0)
    # rb_vels = np.stack(rb_vels, axis=0)
    # rb_vel_invs = np.stack(rb_vel_invs, axis=0)
    # rb_angs = np.stack(rb_angs, axis=0)
    # dof_vels = np.stack(dof_vels, axis=0)
    # dof_poses = np.stack(dof_poses, axis=0)
    ...
    out_path = f"torchready_v5_dof_ar_dance_single.pkl"
    with open(out_path, "wb") as f:
        torch.save(
            {
                "rb_pos": rb_poses,
