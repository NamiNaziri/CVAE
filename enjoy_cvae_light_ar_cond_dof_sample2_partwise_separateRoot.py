...
class Main:
    def __init__(self):
        ...
        self.xs_root = np.concatenate([self.rb_root_vel_inv[...,:2], self.rb_root_ang_vel], axis=-1)
        self.xs = np.concatenate([self.dof_pos[:,self.chains_indecies[0]].reshape(self.dof_pos.shape[0],-1), self.rb_body_vel_inv[:,self.chains_indecies[0]].reshape(self.dof_pos.shape[0],-1)], axis=-1)
        ...
        self.ys_root = self.xs_root * 1
        self.ys = self.xs * 1
        self.ys_upper = self.xs_upper * 1
        #self.ys_lower = self.rb_root_vel_inv * 1

        self.xs_root = torch.tensor(self.xs_root, dtype=torch.float, device="cuda")
        self.ys_root = torch.tensor(self.ys_root, dtype=torch.float, device="cuda")
    ...
    def update(self, delta_t):
        ...
        num_history = 1
        newt = t
        orig_x_root = self.xs_root[[newt]]
        orig_x = self.xs[[newt]]
        orig_x_upper = self.xs_upper[[newt]]
        orig_ys_root = (self.ys_root[newt-num_history: newt])
        ...
        orig_ys = self.cached_decoded[..., 5: 5 + self.ys.shape[-1]] * 1 # + torch.rand_like(self.cached_decoded[..., :self.ys.shape[-1]]) * 0.01
        orig_ys_upper = self.cached_decoded[..., 5 + self.ys.shape[-1]:] * 1
        
        _, decoded, mu, log_var = self.cvae.forward(
            orig_x_root,
            orig_x,
            orig_x_upper,
            orig_ys_root.reshape(1,-1),
            orig_ys.reshape(1,-1),
            orig_ys_upper.reshape(1,-1),
            train_yes=False,
        )
        self.z = mu * 1
        #self.z[...,:3] = ((torch.rand_like(self.z[...,:3]) - 0.5) * 2)* 0
        print(F'min: {torch.min(self.z)}')
        print(F'max: {torch.max(self.z)}')
        #self.z[...,:3] = ((torch.rand_like(self.z[...,:3]) - 0.5) * 2)* 1
