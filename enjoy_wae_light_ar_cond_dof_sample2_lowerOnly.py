...
class Main:
    def __init__(self):
        ...
        self.xs = np.concatenate([self.rb_root_vel_inv[...,:2], self.rb_root_ang_vel,self.dof_pos[:,self.chains_indecies[0]].reshape(self.dof_pos.shape[0],-1)], axis=-1)
        #ys_lower = np.concatenate([xs_root, xs[:,self.chains_indecies[0]].reshape(xs.shape[0], -1 )], axis=-1)
        self.ys = np.concatenate([self.rb_root_vel_inv[...,:2], self.rb_root_ang_vel,self.dof_pos[:,self.chains_indecies[0]].reshape(self.dof_pos.shape[0],-1)], axis=-1)
