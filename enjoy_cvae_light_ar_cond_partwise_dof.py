...
class Main:
    def __init__(self):
        ...
        pkl_file_path = f"torchready_v5_dof_circles.pkl"
        ...
        self.xs =  np.concatenate([self.rb_pos_inv[:, 1:], self.rb_vel_inv[:, 1:],  self.dof_pos], axis=-1)
        #self.xs =  np.concatenate([  self.dof_pos], axis=-1)
        self.xs_root = np.concatenate([self.rb_root_vel_xy, self.rb_root_ang_vel], axis=-1)

        self.ys_lower = np.concatenate([self.xs_root, self.xs[:,chains_indecies[0]].reshape(self.xs.shape[0], -1 )], axis=-1)
        self.ys_lower = np.roll(self.ys_lower, shift=1, axis=0)
    ...
    def update(self):
        ...
        out_xs = torch.cat([orig_x_root, orig_x.reshape(orig_x.shape[0],-1)],dim=-1)
        MSE = torch.nn.functional.mse_loss(out_xs[...,5:].reshape(1,23,9)[...,6:].reshape(-1,1), cvae_decoded[...,5:].reshape(1,23,9)[...,6:].reshape(-1,1).reshape(cvae_decoded.shape[0],-1))
        print(MSE)
        
        cvae_decoded = cvae_decoded.cpu()[...,5:].reshape(1,23,9)[...,6:].reshape(-1,1)
