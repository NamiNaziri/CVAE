...
class Main:
    def __init__(self):
        ...
        pkl_file_path = f"torchready_v5_circle.pkl"
        ...
        self.xs =  self.rb_rot_sixd_inv

        self.xs = self.xs.reshape(self.xs.shape[0],24, -1)
    ...
    def update(self):
        ...
        cvae_decoded = cvae_decoded.reshape((1, 24, -1))

        # cvae_decoded_with_yaw = cvae_decoded
        ...
        '''
        rb_rot_sixd_recon = cvae_decoded[:, :, :6].reshape(-1, 144)
        rb_rot_sixd_recon = rb_rot_sixd_recon

        #TODO add this when we remove the root
        ...
        #recon_rot_sixd_reshaped = self.sixd_add_root(rb_rot_sixd_recon,torch.tensor((self.cached_root_yaw[t][0],0,0)).numpy()).reshape(24, 3, 2)
        rotmat = sixd_to_rotmat(rb_rot_sixd_recon).detach().cpu().numpy()
        rot = Rotation.from_matrix(rotmat.reshape(-1, 3, 3))
        correction = Rotation.from_euler(
            "Z",
            self.rb_rot_euler.reshape(-1,24,3)[t,0,[0]]
            .repeat(24, -1)
            .reshape(-1),
        )  # undo root rotation from all body parts of all frames
        recon_rot = correction * rot
        corrected_rotmat = recon_rot.as_matrix().reshape((-1, 24, 3, 3))
        #new_sixd = corrected_rotmat[..., :2].reshape(-1, 24, 6)
        recon_rot_sixd_reshaped =  corrected_rotmat[..., :2].reshape(-1, 24, 6)
        recon_rot_sixd_reshaped  = recon_rot_sixd_reshaped.reshape(24,3,2)
        ...
        for i in range(len(self.gt_actors)):
            ...
            m[:3, :3] = recon_rotmat
            m[:3, 3] = recon_global_translation[i] * 1
