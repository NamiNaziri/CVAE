...
from torch_nets import PartWiseVAE, ConditionalVAE, ThroughDataset, ConditionalPartWiseVAE
import debugpy
...
def main():
    ...
    xs = dof_pos
    ys_lower = np.concatenate([rb_root_vel_inv], axis=-1)
    ...
    cond_sizes[0] = 3 # lower body: condition on root velocity 
    cond_sizes[1] = len(lower_body) * 3 # upper body: condition on the lower body
    ...
    ys_lower_train = ys_lower[train_idxs] * 1.0
    ...
    ys_lower_valid = torch.as_tensor(ys_lower[valid_idxs], dtype=torch.float)
    ...
    while global_steps_elapsed <= n_total_steps:
        ...
        with torch.no_grad():
            ...
            z, decoded, mu, log_var = model.forward(xs_valid,ys_lower_valid, False)
