...
def main():
    ...
    if(args.use_cache):
        ...
        xs = [np.concatenate([t1[:,chains_indecies[1]].reshape(t1.shape[0],-1)], axis=-1) for (t1,t2,t3) in zip(dof_pos,rb_root_vel_inv,rb_root_ang_vel)]
        #self.rb_root_vel_inv[...,:2], self.rb_root_ang_vel,self.dof_pos.reshape(-1,69)
        ys = [np.concatenate([t1[:,chains_indecies[1]].reshape(t1.shape[0],-1)], axis=-1) for (t1,t2,t3) in zip(dof_pos,rb_root_vel_inv,rb_root_ang_vel)]
