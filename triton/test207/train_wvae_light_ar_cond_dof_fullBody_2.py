    ...
    while global_steps_elapsed <= n_total_steps:
        ...
        if global_steps_elapsed < n_total_steps:
            ...
            for t in range(args.scheduled_sampling_length):
                ...
                z = torch.randn(x_in.size()[0], args.latent_size).cuda() * args.sigma
                d_z = disc(z)

                z_hat = model.encode(x_in,w_in)
                ...
                z_hat, decoded,= model(x_in, w_in, False)  # don't collect logits here
        
                

                z_hat2 = model.encode(x_in.detach(), w_in.detach(),)
