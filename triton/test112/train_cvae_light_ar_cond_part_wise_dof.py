...
ARConditionalPartWiseVAE7,ARConditionalPartWiseVAE8, ARSeparateConditionalPartWiseVAE
# import debugpy
...
def main():
    ...
    xs_lower = [np.concatenate([t2[...,:2], t3 ,t1[:,chains_indecies[0]].reshape(t1.shape[0],-1),t4[:,chains_indecies[0]].reshape(t4.shape[0],-1)], axis=-1) for (t1,t2,t3,t4) in zip(dof_pos,rb_root_vel_inv,rb_root_ang_vel, rb_body_vel_inv)]
    ...
    if args.checkpoint_path is None:
        model = ARSeparateConditionalPartWiseVAE(
            nail_sizes,
        ...
        if epochs_elapsed % eval_every == 0 or global_steps_elapsed >= n_total_steps:
            ...
            with torch.no_grad():
                for t in range(steps):
                    ...
                    y_lower_hats.append(cvae_decoded[...,:y_lower_tar.shape[-1]])
                    y_upper_hats.append(cvae_decoded[...,y_lower_tar.shape[-1]:])

                    #rollout_xins.append(x_in)
                    rollout_youts.append(cvae_decoded)
