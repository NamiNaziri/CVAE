...
#import torch_nets
from torch_nets import PartWiseVAE, ConditionalVAE, ThroughDataset, ConditionalPartWiseVAE, ARConditionalPartWiseVAE, ConditionalVAE2
# import debugpy
...
def main():
    ...
    if(not args.use_cache):
        ...
        pkl_file_path = "torchready_v5_dof_ar_dance.pkl"#f"torchready_v5_dof_1k.pkl"
    ...
    if(args.use_cache):
        ...
        xs_non_zero_stack = torch.cat(xs,dim=0) * 1
        xs_non_zero_stack = xs_non_zero_stack.cuda()
        #ys_non_zero_stack = torch.cat(ys,dim=0)
    ...
    while global_steps_elapsed <= n_total_steps:
        ...
        ratio = min(1, global_steps_elapsed / (n_anneal_steps * 0.75))
        lr = init_lr * (1 - ratio) + end_lr * ratio
        #kl_weights = [0, 0.0001, 0.001,0.002, 0.003, 0.004,0.005,0.006,0.007, 0.008, 0.009, 0.01, 0.02]
        #kl_weights = [0, 0.0001, 0.001,0.002]
        kl_weights = [0.005]
        if(epochs_elapsed == 50):
            steps_after_50_epochs = global_steps_elapsed
        if(epochs_elapsed % 100 == 0):
            #kl_ratio = min(1, (global_steps_elapsed - steps_after_50_epochs) / (n_anneal_steps * 1))
            #kl_weight =  init_kd_weight  * (1 - kl_ratio) + args.kld_weight * kl_ratio
            kl_weight = kl_weights[min(epochs_elapsed//100, len(kl_weights)-1)]
        ...
        if epochs_elapsed == 0:
            model.nail_rms.update(xs_non_zero_stack.reshape(-1, xs_non_zero_stack.shape[-1]))
            #NOTE ys is the same as xs
            model.cond_rms.update(xs_non_zero_stack.reshape(-1, xs_non_zero_stack.shape[-1]))
