    ...
    while global_steps_elapsed <= n_total_steps:
        ...
        if global_steps_elapsed < n_total_steps:
            ...
            # for i, (x,y, gt) in enumerate(dataloader):
                ...
                z_hat2 = torch.clamp(z_hat2, -10, 10)
                d_z_hat = disc(z_hat2)
            ...
            epoch_num += 1
            epochs_elapsed += 1
            writer.add_scalar("train/d_loss", (torch.log(d_z_hat)).mean(), global_steps_elapsed)
            writer.add_scalar("train/loss", (MSE + (torch.log(d_z_hat)).mean()).item(), global_steps_elapsed)
            writer.add_scalar("train/MSE", (MSE).item(), global_steps_elapsed)
            writer.add_scalar("train/lr", lr, global_steps_elapsed)
            writer.add_scalar("train/d_loss/epochs", (torch.log(d_z_hat)).mean(), epochs_elapsed)
            ...
            writer.add_scalar(
                    "train/loss/epochs", (MSE + (torch.log(d_z_hat)).mean()).item(), epochs_elapsed
                )
            logger.info(
                f"Epoch {epochs_elapsed}:\tMSE: {MSE.item():.3f}\tdisc_loss: {d_loss.item():.3f} \t SS: {args.scheduled_sampling_length:.3f} \t mu: {z_hat2.mean().item():.2e} sigma: {z_hat2.std().item():.2e}"
            )
        else:
            break
