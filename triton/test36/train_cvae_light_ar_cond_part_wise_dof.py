...
def main():
    ...
    #xs = np.concatenate([ dof_pos], axis=-1)
    #xs_root = np.concatenate([rb_root_vel_xy, rb_root_ang_vel], axis=-1)
    xs_root = [np.concatenate([t1[...,:2], t2], axis=-1) for (t1,t2) in zip(rb_root_vel_inv,rb_root_ang_vel)]
    ...
    for chain_idx, chain in enumerate(chains):
        ...
        nail_sizes.append(len(chain) *  xs_root[0].shape[-1]) # temp, will rewrite this blow

    
    cond_sizes = nail_sizes * 1

    nail_sizes[0] = xs_root[0].shape[-1] 
    nail_sizes[1] = ys_lower[0].shape[-1] #input is the output of the lowerbody decoder = root + lower body

    
    cond_sizes[0] =  ys_lower[0].shape[-1] # condition on prev root + lower body
    cond_sizes[1] = ys_upper[0].shape[-1] # condition on prev upper body
    
    out_sizes = cond_sizes * 1
    ...
    n_train = int(xs_root.shape[0] * 0.99)
    ...
    train_idxs = torch.tensor(np.arange(xs_root.shape[0]-1))
    ...
    valid_idxs = torch.tensor([xs_root.shape[0] - 1])
    xs_root_train = xs_root.cuda() #TODO use train_idx when we have validation. the first index should be the animation idx not the frame idx!
        ...
        if epochs_elapsed % eval_every == 0 or global_steps_elapsed >= n_total_steps:
            ...
            with torch.no_grad():
                for t in range(steps):
                    x_in = xs_root_train[it[:, 0], it[:, 1] + t]
                    ...
                    z, decoded, mu, log_var = model(x_in, w_lower_in, w_upper_in, x_in, False)
        ...
        if global_steps_elapsed < n_total_steps:
            ...
            with torch.no_grad():
                for t in range(args.scheduled_sampling_length):
                    ...
                    z, decoded, mu, log_var = model(x_in, w_lower_in, w_upper_in, x_in, False)  # don't collect logits here
            ...
            for i, (x_root,y_lower,y_upper, gt) in enumerate(dataloader):
                ...
                z, decoded, mu, log_var = model(x_root,y_lower, y_upper,x_root, False)
                KLD = -0.5 * torch.mean(1 + log_var - mu.pow(2) - log_var.exp())
