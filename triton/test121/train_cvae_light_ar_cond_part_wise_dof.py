...
ARConditionalPartWiseVAE7,ARConditionalPartWiseVAE8, ARSeparateConditionalPartWiseVAE, ARConditionalSeparateRootPartWiseVAE
# import debugpy
...
def main():
    ...
    if(not args.use_cache):
        ...
        logger.info('hiii')
        pkl_file_path = "torchready_v5_dof_ar_1.pkl"#f"torchready_v5_dof_1k.pkl"
        data = torch.load(pkl_file_path)
        logger.info('first')
        pkl_file_path_list = [
            
            "torchready_v5_dof_ar_2.pkl",
            # "torchready_v5_dof_ar_3.pkl",
            # "torchready_v5_dof_ar_4.pkl",
        ]
        
        for path in pkl_file_path_list:
            logger.info(path)
    ...
    xs_root= [np.concatenate([t2[...,:2], t3 ], axis=-1) for (t2,t3) in zip(rb_root_vel_inv,rb_root_ang_vel)]
    xs_lower = [np.concatenate([t1[:,chains_indecies[0]].reshape(t1.shape[0],-1),t4[:,chains_indecies[0]].reshape(t4.shape[0],-1)], axis=-1) for (t1,t2,t3,t4) in zip(dof_pos,rb_root_vel_inv,rb_root_ang_vel, rb_body_vel_inv)]
    
    xs_upper = [np.concatenate([t1[:,chains_indecies[1]].reshape(t1.shape[0],-1), t4[:,chains_indecies[1]].reshape(t4.shape[0],-1)], axis=-1) for (t1,t2,t3,t4) in zip(dof_pos,rb_root_vel_inv,rb_root_ang_vel,rb_body_vel_inv)]
    logger.info('here2')
    ...
    ys_lower = xs_lower * 1 #[np.concatenate([t2[...,:2], t3 ,t1[:,chains_indecies[0]].reshape(t1.shape[0],-1),t4[:,chains_indecies[0]].reshape(t4.shape[0],-1)], axis=-1) for (t1,t2,t3,t4) in zip(dof_pos,rb_root_vel_inv,rb_root_ang_vel, rb_body_vel_inv)]

     # upper is conditioned on upper body
    ys_upper = xs_upper * 1 #[np.concatenate([t1[:,chains_indecies[1]].reshape(t1.shape[0],-1)], axis=-1) for (t1,t2,t3) in zip(dof_pos,rb_root_vel_inv,rb_root_ang_vel)]

    xs_root = [t.reshape(t.shape[0], -1) for t in xs_root]
    xs_root = [torch.tensor(t, dtype=torch.float) for t in xs_root]
    ...
    ys_root = [t.reshape(t.shape[0], -1) for t in ys_root]
    ys_root = [torch.tensor(t, dtype=torch.float) for t in ys_root]
    ...
    xs_root_nanpads = [
        ...
        for (l,t) in zip(lengths_to_go, xs_root)
    ]
    ...
    ys_root_nanpads = [
        torch.ones((l, t.shape[-1]))* torch.nan
        for (l,t) in zip(lengths_to_go, ys_root)
    ]
    ...
    xs_root_stack = torch.stack([torch.cat([t1,t2], dim = 0) for (t1,t2) in zip(xs_root, xs_root_nanpads)], dim =0)
    ...
    ys_root_stack = torch.stack([torch.cat([t1,t2], dim = 0) for (t1,t2) in zip(ys_root, ys_root_nanpads)], dim =0)
    ...
    #initializing with the root specific AE settings
    hidden_sizes = [128]
    latent_sizes = [3]
    nail_sizes = [xs_root[0].shape[-1]]
    for chain_idx, chain in enumerate(chains):
    ...
    nail_sizes[1] = xs_lower[0].shape[-1] 
    nail_sizes[2] = xs_upper[0].shape[-1]

    
    cond_sizes[1] =  ys_lower[0].shape[-1] + 5 # condition on prev root + lower body
    cond_sizes[2] = ys_upper[0].shape[-1] + ys_lower[0].shape[-1] # condition on prev upper body + current lower body
    ...
    if args.checkpoint_path is None:
        model = ARConditionalSeparateRootPartWiseVAE(
            nail_sizes,
    ...
    while global_steps_elapsed <= n_total_steps:
        ...
        if epochs_elapsed % eval_every == 0 or global_steps_elapsed >= n_total_steps:
            ...
            y_hat_root_start = torch.zeros_like(ys_root_train[it[:, 0], it[:, 1]])
            ...
            y_hat_lower_start = torch.zeros_like(ys_lower_train[it[:, 0], it[:, 1]])
            ...
            with torch.no_grad():
                for t in range(steps):
                    x_root_in = xs_root_train[it[:, 0], it[:, 1] + t]
                    x_lower_in = xs_lower_train[it[:, 0], it[:, 1] + t]
                    x_upper_in = xs_upper_train[it[:, 0], it[:, 1] + t]

                    w_root_in = torch.stack(y_root_hats[-args.num_history :], 1)
                    w_root_in = w_root_in.reshape(w_root_in.shape[0], -1)
                    ...
                    z, decoded, mu, log_var = model(x_root_in, x_lower_in,x_upper_in, w_root_in,w_lower_in, w_upper_in, False)  # don't collect logits here
                    # y_tar = train_y[it[:, 0], it[:, 1] + t + 1][:, None]

                    # current_start = 0
                    # decodeds=[]
                    # for index, hs in enumerate(model.hammer_size):
                    #     if(index == 0):
                    #         ys = w_lower_in
                    #     else:
                    #         ys = w_upper_in
                    ...
                    #     current_start += hs
                    #     decodeds.append(model.decode(chain_z,ys, index))
                    # cvae_decoded = torch.concatenate(decodeds, axis=-1)

                    y_root_hats.append(decoded[..., :5])
                    y_lower_hats.append(decoded[...,5 :y_lower_tar.shape[-1] + 5])
                    y_upper_hats.append(decoded[...,5 + y_lower_tar.shape[-1]:])


                    #rollout_xins.append(x_in)
                    rollout_youts.append(decoded)
                    rollout_gts.append(torch.concatenate([y_root_tar, y_lower_tar,y_upper_tar],-1))
        ...
        if ((epochs_elapsed > 100) and (epochs_elapsed % 1005 == 0)):
            args.scheduled_sampling_length = min(args.scheduled_sampling_length + 1 , 10)

        logger.info(
            f"Epoch {epochs_elapsed}:\tMSE: {Eval_MSE.item():.3f}\tKLD: {Eval_KLD.item():.3f} \t SS: {args.scheduled_sampling_length:.3f}"
        )
        ...
        if global_steps_elapsed < n_total_steps:
            ...
            rollout_root_xins = torch.concatenate(rollout_root_xins, dim=0)
            ...
            rollout_root_wins = torch.concatenate(rollout_root_wins, dim=0)
            ...
            for i, (x_root, x_lower, x_upper,y_root,y_lower,y_upper, gt) in enumerate(dataloader):
                ...
                z, decoded, mu, log_var = model(x_root, x_lower, x_upper,y_root, y_lower, y_upper, True)
