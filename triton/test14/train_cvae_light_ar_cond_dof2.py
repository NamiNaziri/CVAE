...
def main():
    ...
    while global_steps_elapsed <= n_total_steps:
        ...
        if epochs_elapsed % save_every == 0 or global_steps_elapsed >= n_total_steps:
            ...
            batch_valid_idxs = torch.arange(len(valid_idxs)).repeat_interleave(lengths[valid_idxs.tolist()]- 10 - 2 ).to(dtype=torch.long)
            ...
            t_start = torch.cat([torch.arange(length- 10 - 2) + 1 for length in valid_lengths]).to(dtype=torch.long)
