...
class A2CBuilder(NetworkBuilder):
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            # self.adapter_mlp = deepcopy(self.actor_mlp)
            # self.adapter_mlp[0] = nn.Linear(13, self.adapter_mlp[0].out_features)
            # self.adapter_mlp = self.adapter_mlp.to(next(self.actor_mlp.parameters()).device)
            # self.adapter_rms = RunningMeanStd((13,))
            # self.adapter_rms = self.adapter_rms.to(next(self.actor_mlp.parameters()).device)
        ...
        def forward(self, obs_dict):
            ...
            if self.separate:
                ...
                if self.has_rnn:
                    ...
                    out = self.actor_mlp(out)
                    # out1 = self.actor_mlp(out1)
                    # if out2.shape[-1] > 0:
                    #     out2 = self.adapter_mlp(self.adapter_rms(out2))
                    #     out = out1 + out2
                    # else:
                    #     out = out1
