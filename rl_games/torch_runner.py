...
from isaacgymenvs.utils import my_agent
...
class Runner:
    def __init__(self, algo_observer=None):
        ...
        # )
        self.algo_factory.register_builder(
            "a2c_continuous", lambda **kwargs: my_agent.MyAgent(**kwargs)
        )
        self.algo_factory.register_builder(
        ...
        self.player_factory.register_builder(
            "a2c_continuous", lambda **kwargs: players.PpoPlayerContinuous(**kwargs)
        )
        self.player_factory.register_builder(
    ...
    def load_config(self, params):
        ...
        self.config["run_name"] = params["run_name"]
        self.config["out_name"] = params["out_name"]
        ...
        if has_rnd_net:
            ...
            network = self.model_builder.network_factory.create(
                params["config"]["rnd_config"]["network"]["name"]
            )
    ...
    def load(self, yaml_conf):
        ...
        default_config["run_name"] = yaml_conf["run_name"]
        default_config["out_name"] = yaml_conf["out_name"]
    ...
    def run_train(self):
        ...
        if self.exp_config:
            self.experiment = experiment.Experiment(
                self.default_config, self.exp_config
            )
            exp_num = 0
            ...
            while exp is not None:
                ...
                agent = self.algo_factory.create(
                    self.algo_name, base_name="run", config=self.config
                )
            ...
            self.reset()
            default_config = copy.deepcopy(self.default_config)
            default_config["run_name"] = self.config["run_name"]
            default_config["out_name"] = self.config["out_name"]
            self.load_config(default_config)
    ...
    def create_agent(self, obs_space, action_space):
        return self.algo_factory.create(
            self.algo_name,
            base_name="run",
