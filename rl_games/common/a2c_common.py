...
class A2CBase:
    def __init__(self, base_name, config):
        ...
        self.nn_dir = os.path.join(f"{proj_dir}", "out", config["run_name"], config["out_name"])
        self.summaries_dir = os.path.join(f"{proj_dir}", "logdir", config["run_name"], config["out_name"])
    ...
    def get_action_values(self, obs):
        # processed_obs = obs['obs'] * 1
        processed_obs = self._preproc_obs(obs['obs'])
        # processed_obs[:, :358] = self._preproc_obs(obs['obs'][:, :358])
        self.model.eval()
        input_dict = {
            'is_train': False,
            'prev_actions': None, 
    ...
    def set_full_state_weights(self, weights):
        ...
        try:
            ...
            print('Optimizer state dict not found in checkpoint. Initializing optimizer from scratch')
    ...
    def set_weights(self, weights):
        # if "a2c_network.sigma.weight" not in weights['model']:
        #     weights['model']["a2c_network.sigma.weight"] = self.model.state_dict()["a2c_network.sigma.weight"]
        #     weights['model']["a2c_network.sigma.bias"] = self.model.state_dict()["a2c_network.sigma.bias"]
        self.model.load_state_dict(weights['model'])
        self.set_stats_weights(weights)
