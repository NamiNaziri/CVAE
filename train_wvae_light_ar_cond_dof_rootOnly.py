...
def main():
    ...
    if(args.use_cache):
        ...
        ys = [np.concatenate([t2[...,:2], t3], axis=-1) for (t1,t2,t3) in zip(dof_pos,rb_root_vel_inv,rb_root_ang_vel)]
    ...
    while global_steps_elapsed <= n_total_steps:
        ...
        if epochs_elapsed % eval_every == 0 or global_steps_elapsed >= n_total_steps:
            if epochs_elapsed % save_every == 0 or global_steps_elapsed >= n_total_steps:
                ...
                save_path = f"{out_dir}/wae_{epochs_elapsed:06d}.pkl"
                ...
                save_path = f"{out_dir}/disc_{epochs_elapsed:06d}.pkl"
                #torch.save(d, save_path)
        ...
        if global_steps_elapsed < n_total_steps:
            ...
            with torch.no_grad():
                for t in range(args.scheduled_sampling_length):
                    ...
                    if(epochs_elapsed < 25 or (epochs_elapsed < 50 and random.uniform(0, 1) < 0.5)):
            ...
            for epoch in range(args.sample_train_epoch):
                ...
                # for i, (x,y, gt) in enumerate(dataloader):
                    ...
                    z_hat2 = model.encode(x.detach(),y.detach())
                    ...
                    if global_steps_elapsed >= n_total_steps:
                        break
                mini_epochs_elapsed += 1
                logger.info(
                f"Mini Epoch {mini_epochs_elapsed}:\tMSE: {MSE.item():.3f}\tdisc_loss: {criterion(d_z_hat2, torch.ones_like(d_z_hat2)).item():.3f} \t SS: {args.scheduled_sampling_length:.3f} \t mu: {z_hat2.mean().item():.2e} sigma: {z_hat2.std().item():.2e}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample_train_epoch", type=float, default=10)
