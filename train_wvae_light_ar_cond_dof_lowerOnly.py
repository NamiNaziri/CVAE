...
def main():
    ...
    if(args.use_cache):
        ...
        xs = [np.concatenate([t2[...,:2], t3 ,t1[:,chains_indecies[0]].reshape(t1.shape[0],-1)], axis=-1) for (t1,t2,t3) in zip(dof_pos,rb_root_vel_inv,rb_root_ang_vel)]
        #self.rb_root_vel_inv[...,:2], self.rb_root_ang_vel,self.dof_pos.reshape(-1,69)
        ys = [np.concatenate([t2[...,:2], t3 ,t1[:,chains_indecies[0]].reshape(t1.shape[0],-1)], axis=-1) for (t1,t2,t3) in zip(dof_pos,rb_root_vel_inv,rb_root_ang_vel)]
    ...
    while global_steps_elapsed <= n_total_steps:
        ...
        if global_steps_elapsed < n_total_steps:
            ...
            for i, (x,y, gt) in enumerate(dataloader):
                ...
                d_z_loss = args.lambda_val *torch.log(d_z).mean()
                d_z_hat_loss = args.lambda_val *torch.log(1 - d_z_hat).mean()
            ...
            epoch_num += 1
            epochs_elapsed += 1
            writer.add_scalar("train/d_loss", d_loss, global_steps_elapsed)
            writer.add_scalar("train/loss", (MSE + d_loss).item(), global_steps_elapsed)
            writer.add_scalar("train/MSE", (MSE).item(), global_steps_elapsed)
            writer.add_scalar("train/lr", lr, global_steps_elapsed)
            writer.add_scalar("train/d_loss/epochs", d_loss, epochs_elapsed)
            writer.add_scalar("train/schedule_sampling/epochs", args.scheduled_sampling_length, epochs_elapsed)
            writer.add_scalar(
                    "train/loss/epochs", (MSE + d_loss).item(), epochs_elapsed
                )
            logger.info(
                f"Epoch {epochs_elapsed}:\tMSE: {MSE.item():.3f}\tKLD: {d_loss.item():.3f} \t SS: {args.scheduled_sampling_length:.3f}"
            )
        else:
            break
