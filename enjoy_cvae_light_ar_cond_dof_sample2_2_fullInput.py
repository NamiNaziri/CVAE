...
def decode(cvae, z: torch.Tensor, y: torch.Tensor):
    decoder_out = z * 1
    #y = y + torch.rand_like(y, device='cuda') 
    y = cvae.cond_rms.normalize(y * 1)
    
    for j, op in enumerate(cvae.decoder):
...
class Main:
    def __init__(self):
        ...
        #pkl_file_path = "torchready_v5_dof_ar_dance.pkl"
        pkl_file_path = "torchready_v5_dof_ar_dance_single.pkl"
        ...
        self.rb_root_ang_vel = self.rb_ang_vel[..., 0, [2]]
        ...
        self.rb_body_pos_inv = self.rb_pos_inv[:, 1:] * 1
        ...
        self.xs = np.concatenate([self.rb_root_vel_inv[...,:2], self.rb_root_ang_vel,self.dof_pos.reshape(self.dof_pos.shape[0],-1), self.rb_body_vel_inv.reshape(self.rb_body_vel_inv.shape[0],-1), self.rb_body_pos_inv.reshape(self.rb_body_pos_inv.shape[0], -1)], axis=-1)
        #self.xs = np.concatenate([self.rb_root_vel_inv[...,:2], self.rb_root_ang_vel,self.dof_pos.reshape(self.dof_pos.shape[0],-1)], axis=-1)
        #ys_lower = np.concatenate([xs_root, xs.reshape(xs.shape[0], -1 )], axis=-1)
        self.ys = self.xs * 1#np.concatenate([self.rb_root_vel_inv[...,:2], self.rb_root_ang_vel,self.dof_pos.reshape(self.dof_pos.shape[0],-1)], axis=-1)
        ...
        point_cloud_plane = np.stack([xx.reshape(-1), yy.reshape(-1)]).T
        point_cloud_plane = np.concatenate([point_cloud_plane, np.zeros((point_cloud_plane.shape[0], 1))], axis=-1)
        point_cloud_plane = pv.PolyData(point_cloud_plane)
        self.pl.add_mesh(
            # pv.Cube(center=(0, 0, -0.5), x_length=100, y_length=100, z_length=0),
            point_cloud_plane.delaunay_2d(), show_edges=True
        )
    ...
    def update(self, delta_t):
        ...
        if self.current_frame >= self.end_frame:
            tensor_stack = torch.stack(self.z_list)
            mean = torch.mean(tensor_stack[:,0], dim=0)
            variance = torch.var(tensor_stack[:,0], dim=0)
        ...
        if(self.cached_decoded is None):
            self.cached_decoded =  orig_ys.reshape(num_history, -1) 
        orig_ys = self.cached_decoded

        z, decoded, mu, log_var = self.cvae.forward(
            orig_x,
            orig_ys.reshape(1,-1),
            train_yes=False,
        )
        ...
        self.z = (torch.randn_like(z)) 
        #print(z)
        # if(self.z is None or (self.current_frame % 1 == 0)):
        #     self.z = (torch.randn_like(z))
        #     print('change latent')
        # print(F'min: {torch.min(self.z)}')
        # print(F'max: {torch.max(self.z)}')
        if(self.update_hands):
            # tensor_stack = torch.stack(self.z_list)
            # mean = torch.mean(tensor_stack[:,0], dim=0)
            # variance = torch.var(tensor_stack[:,0], dim=0)
            # self.z = (torch.randn_like(self.z) + mean) * variance
            self.z = (torch.randn_like(self.z))
        #self.z = ((torch.rand_like(self.z) - 0.5) * 2)* 1
...
#############
        ...
        self.z[..., 0] += self.engine.get_val("dim_0")
        ...
        if self.z.shape[-1] > 5:
            ...
            self.z[..., 10] += self.engine.get_val("dim_10")
            self.z[..., 11] += self.engine.get_val("dim_11")
            self.z[..., 12] += self.engine.get_val("dim_12")
            self.z[..., 13] += self.engine.get_val("dim_13")
            self.z[..., 14] += self.engine.get_val("dim_14")

        cvae_decoded = self.cvae.decode(self.z, orig_ys.reshape(1,-1),)
        #cvae_decoded = decode(self.cvae, self.z, orig_ys.reshape(1,-1),)
        ...
        if(self.cached_decoded is not None):
            ...
            self.cached_decoded[-1] = cvae_decoded.detach() * 1

        MSE = torch.nn.functional.mse_loss(self.ys[[t]].reshape(1,-1), cvae_decoded.reshape(1,-1))
        #print(MSE)

        decoded_root = cvae_decoded[...,:3].detach().cpu().numpy()
        cvae_decoded = cvae_decoded[...,3: 3 + 69].reshape((1, 23, -1)).detach().cpu().numpy()
    ...
    def define_sliders(self):
        ...
        self.pl.add_slider_widget(
            lambda value: self.engine("dim_10", (value)),
            [start_val, end_val],
            title="10 dim",
            pointa=(0.025, 0.66),
            pointb=(0.31, 0.66),
        ...
        self.pl.add_slider_widget(
            lambda value: self.engine("dim_11", (value)),
            [start_val, end_val],
            title="11 dim",
            pointa=(0.025, 0.54),
            pointb=(0.31, 0.54),
        ...
        self.pl.add_slider_widget(
            lambda value: self.engine("dim_12", (value)),
            [start_val, end_val],
            title="12 dim",
            pointa=(0.67, 0.9),
            pointb=(0.98, 0.9),
        ...
        self.pl.add_slider_widget(
            lambda value: self.engine("dim_13", (value)),
            [start_val, end_val],
            title="13 dim",
            pointa=(0.67, 0.78),
            pointb=(0.98, 0.78),
        ...
        self.pl.add_slider_widget(
            lambda value: self.engine("dim_14", (value)),
            [start_val, end_val],
            title="14 dim",
            pointa=(0.67, 0.66),
            pointb=(0.98, 0.66),
