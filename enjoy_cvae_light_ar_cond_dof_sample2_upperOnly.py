...
class Main:
    def __init__(self):
        ...
        for key, value in data.items():
            ...
            data[key]= data[key][1033].squeeze()#1033
        ...
        self.xs = np.concatenate([self.dof_pos[:,self.chains_indecies[1]].reshape(self.dof_pos.shape[0],-1)], axis=-1)
        #ys_lower = np.concatenate([xs_root, xs[:,self.chains_indecies[0]].reshape(xs.shape[0], -1 )], axis=-1)
        self.ys = np.concatenate([self.dof_pos[:,self.chains_indecies[1]].reshape(self.dof_pos.shape[0],-1)], axis=-1)
...
#############
        ...
        # cvae_decoded[..., 0] = add_x_vel
        # cvae_decoded[..., 1] = add_y_vel
        # cvae_decoded[..., 2:5] = 0
        ...
        cvae_decoded = cvae_decoded.reshape((1, len(self.chains_indecies[1]), -1)).detach().cpu().numpy()
        cvae_decoded = np.concatenate([self.dof_pos[[[0]], self.chains_indecies[0]],cvae_decoded], axis=-2)
        ...
        if(self.current_root_rot is None):
            self.new_root_rot = self.root_rot[self.selected_valid_idxs[t]] 
            self.current_root_rot = self.new_root_rot * 0
        # else:
        ...
        #root_rot[...,3] = 1
        pose_quat = np.concatenate([self.new_root_rot[None, None, :], pose_quat], axis=1)
        ...
        if(self.current_root_pos is None):
            ...
            correction = Rotation.from_euler(
                "Z",
                Rotation.from_quat(self.new_root_rot)
                .as_euler("zyx")[..., [0]]
                .reshape(-1),
            )
