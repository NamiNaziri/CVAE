...
class Main:
    def __init__(self):
        ...
        self.xs_upper = np.concatenate([self.dof_pos[:,self.chains_indecies[1]].reshape(self.dof_pos.shape[0],-1)], axis=-1)
        ...
        self.ys_upper = np.concatenate([self.dof_pos[:,self.chains_indecies[1]].reshape(self.dof_pos.shape[0],-1)], axis=-1)
    ...
    def update(self, delta_t):
        ...
        if(self.update_hands):
            self.z[...,:3] += ((torch.rand_like(self.z[...,:3]) - 0.5) * 2)* 4
