...
#import torch_nets
from torch_nets import PartWiseVAE, ConditionalWAE, ThroughDataset, WAEDisc, frozen_params, free_params
# import debugpy
...
def main():
    ...
    if(not args.use_cache):
        ...
        # data1 = t`orch.load(pkl_file_path)
    ...
    if(args.use_cache):
        ...
        xs = [np.concatenate([t2[...,:2], t3 ,t1.reshape(t1.shape[0],-1)], axis=-1) for (t1,t2,t3) in zip(dof_pos,rb_root_vel_inv,rb_root_ang_vel)]
        #self.rb_root_vel_inv[...,:2], self.rb_root_ang_vel,self.dof_pos.reshape(-1,69)
        ys = [np.concatenate([t2[...,:2], t3 ,t1.reshape(t1.shape[0],-1)], axis=-1) for (t1,t2,t3) in zip(dof_pos,rb_root_vel_inv,rb_root_ang_vel)]
    ...
    if args.checkpoint_path is None:
        ...
        dis_optim = Adam(disc.parameters(), init_lr)
    ...
    while global_steps_elapsed <= n_total_steps:
        ...
        if epochs_elapsed % eval_every == 0 or global_steps_elapsed >= n_total_steps:
            ...
            with torch.no_grad():
                for t in range(steps):
                    ...
                    z, decoded,= model(x_in, w_in, False)
                ...
                eval_normal_z = torch.randn(x_in.size()[0], args.latent_size).cuda() * args.sigma
                eval_normal_dic = disc(eval_normal_z)

                #exp_z = disc(exponential_random_like = torch.distributions.Exponential(rate=1.0).sample(eval_normal_z.shape))
                ...
                # writer.add_scalar("train/eval_exp_dic_val", torch.sigmoid(exp_z).mean(), global_steps_elapsed)
                # writer.add_scalar("valid/eval_exp_dic_error", criterion(exp_z, torch.zeros_like(exp_z)).item(), global_steps_elapsed)
                writer.add_scalar("train/eval_normal_dic_val", torch.sigmoid(eval_normal_dic).mean(), global_steps_elapsed)
                writer.add_scalar("valid/eval_normal_dic_error", criterion(eval_normal_dic, torch.ones_like(eval_normal_dic)).item(), global_steps_elapsed)
        ...
        if global_steps_elapsed < n_total_steps:
            ...
            # train_lengths = lengths[train_idxs] #TODO: this is temp for single animation with length of 800
            # t_start = (
            #     torch.rand((args.batch_size,))
            #     * (train_lengths - args.scheduled_sampling_length - 2) # making sure it won't go out of animation length
            #     + 1
            # ).to(dtype=torch.long)

            # it = torch.stack([train_idxs, t_start], dim=-1)
            ...
            with torch.no_grad():
                for t in range(args.scheduled_sampling_length):
                    ...
                    z, decoded,= model(x_in, w_in, False)  # don't collect logits here
            ...
            if epochs_elapsed == 0:
                model.input_rms.update(xs_train.reshape(-1, xs_train.shape[-1]))
            ...
            for i, (x,y, gt) in enumerate(dataloader):
                ...
                frozen_params(model)
                free_params(disc)
                model.input_rms.requires_grad_(False)


                #TODO prbably should use shape[0]
                z = torch.randn(x.size()[0], args.latent_size).cuda() * args.sigma
                d_z = disc(z)

                z_hat, _ = model.forward(x,y, False)
                z_hat = torch.clamp(z_hat, -5, 5)
                d_z_hat = disc(z_hat)

                # d_z_loss = torch.log(d_z).mean()
                # d_z_hat_loss = torch.log(1 - d_z_hat).mean()
                d_z_loss = criterion(d_z, torch.ones_like(d_z))
                d_z_hat_loss = criterion(d_z_hat, torch.zeros_like(d_z_hat))
                ...
                z_hat, decoded = model(x,y, False)

                z_hat2,_ = model.forward(x,y, False)
                z_hat2 = torch.clamp(z_hat2, -5, 5)
                d_z_hat2 = disc(z_hat2)

                MSE = torch.nn.functional.mse_loss(decoded, gt)

                # calculate discriminator loss
                #d_loss = args.lambda_val * (torch.log(d_z_hat)).mean()
                d_loss = args.lambda_val * criterion(d_z_hat2, torch.ones_like(d_z_hat2))
            ...
            epoch_num += 1
            epochs_elapsed += 1
            writer.add_scalar("train/d_loss", criterion(d_z_hat2, torch.ones_like(d_z_hat2)), global_steps_elapsed)
            writer.add_scalar("train/loss", (MSE + criterion(d_z_hat2, torch.ones_like(d_z_hat2))).item(), global_steps_elapsed)
            writer.add_scalar("train/d_out", torch.sigmoid(d_z_hat2).mean(), global_steps_elapsed)
            writer.add_scalar("train/MSE", (MSE).item(), global_steps_elapsed)
            writer.add_scalar("train/lr", lr, global_steps_elapsed)
            writer.add_scalar("train/d_loss/epochs", criterion(d_z_hat2, torch.ones_like(d_z_hat2)), epochs_elapsed)
            ...
            writer.add_scalar(
                    "train/loss/epochs", (MSE + criterion(d_z_hat2, torch.ones_like(d_z_hat2))).item(), epochs_elapsed
                )
            logger.info(
                f"Epoch {epochs_elapsed}:\tMSE: {MSE.item():.3f}\tdisc_loss: {criterion(d_z_hat2, torch.ones_like(d_z_hat2)).item():.3f} \t SS: {args.scheduled_sampling_length:.3f} \t mu: {z_hat2.mean().item():.2e} sigma: {z_hat2.std().item():.2e}"
            )
        else:
            break
