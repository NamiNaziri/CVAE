...
class Main:
    def __init__(self):
        ...
        pkl_file_path = f"torchready_v5_Allcircle.pkl"
        ...
        self.xs = np.concatenate(
            [
                self.rb_rot_sixd_inv[self.selected_valid_idxs].reshape(-1, 24, 6)[
                    :, 1:
                ],
                # self.rb_ang_vel[self.selected_valid_idxs].reshape(-1, 24, 3)[:, 1:],
            ],
            axis=-1,
        )
    ...
    def update(self):
        ...
        cvae_decoded = cvae_decoded.reshape((1, 24, -1)).detach().cpu().numpy()[..., :3]
